...
// gcc -Wall -g ./ch347_demo.c -I. -lch347_new -L.
// dmesg | grep hidraw
//... hid-generic 0003:1A86:55DC.0008: hiddev1,hidraw3: USB HID v1.00 Device [wch.cn HID To UART+SPI+I2C] on usb-0000:00:14.0-1.4/input0
//... hid-generic 0003:1A86:55DC.0009: hiddev2,hidraw4: USB HID v1.00 Device [wch.cn HID To UART+SPI+I2C] on usb-0000:00:14.0-1.4/input1
// sudo LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH ./a.out /dev/hidraw3 (uart)
// sudo LD_LIBRARY_PATH=.:$LD_LIBRARY_PATH ./a.out /dev/hidraw4 (subtest spi/i2c/gpio/quit -> f/e/g/q )
...
{
   int fd;
unsigned char version[100]; //darioia:  added unsigned
   CHIP_TYPE chiptype;
   uint32_t dev_id;
...
unsigned int Flash_Block_Read(unsigned int address, uint8_t *pbuf, unsigned int len)
{
   int iChipSelect;
   int iLength;
unsigned   int oLength; //darioia: Added unsigned
...
{
   int i;
unsigned int oReadLength;// darioia:  added unsigned
   uint8_t retue[32] = { 0 };
...
{
   /*unsigned */char buf[256]; //darioia: commented unsigned
   int ret;
