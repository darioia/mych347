...
#include "CH347LIB.h"
// gcc -Wall -g -I/usr/include/hidapi -lhidapi-hidraw ./src/CH347PAR_Demo.c -I./include -lch347 -L./lib
// sudo ./CH347PAR_Demo
...
void infoDevice(int fd)
{
mDeviceInforS DevInformation;

if (true == CH347GetDeviceInfor(fd/*iIndex*/, &DevInformation))
{
   printf("Index=%d\n", DevInformation.iIndex);
   /*
    UCHAR DevInformation.UsbClass;             // 0:CH347_USB_CH341, 2:CH347_USB_HID,3:CH347_USB_VCP
    UCHAR DevInformation.FuncType;             // 0:CH347_FUNC_UART,1:CH347_FUNC_SPI_IIC,2:CH347_FUNC_JTAG_IIC
    int DevInformation.DevHandle;
    USHORT DevInformation.BulkOutEndpMaxSize;  // Upload endpoint size
    USHORT DevInformation.BulkInEndpMaxSize;   // The size of the downstream endpoint     
    UCHAR DevInformation.DataUpEndp;           // The endpoint address
    UCHAR DevInformation.DataDnEndp;           // The endpoint address
    ULONG DevInformation.WriteTimeout;         // USB write timeout
    ULONG DevInformation.ReadTimeout;          // USB read timeout
    CHAR DevInformation.FuncDescStr[64];       // Interface functions
    */
//printf(" =%d\n", DevInformation. );

printf("FuncType =%d\n", DevInformation.FuncType);// 0:CH347_FUNC_UART,1:CH347_FUNC_SPI_IIC,2:CH347_FUNC_JTAG_IIC
printf("DeviceID =%s\n", DevInformation.DeviceID ); // USB\VID_xxxx&PID_xxxx
printf("ChipMode =%d\n", DevInformation.ChipMode); // Chip Mode, 0:Mode0(UART0/1); 1:Mode1(Uart1+SPI+IIC);  2:Mode2(HID Uart1+SPI+IIC) 3:Mode3(Uart1+Jtag+IIC)
printf("CH347IfNum=%d\n", DevInformation.CH347IfNum);// USB interface number: 0:UART,1:SPI/IIC/JTAG/GPIO
printf("UsbSpeedType=%d\n", DevInformation.UsbSpeedType); // USB Speed type,0:FS,1:HS,2:SS
printf("CmdDataMaxSize=%ld\n", DevInformation.CmdDataMaxSize);

printf("DevicePath =%s\n", DevInformation.DevicePath );// MAX_PATH bytes Device link name, used for CreateFile
printf("ProductString =%s\n", DevInformation.ProductString );// 64 bytes Product string in USB descriptor
printf("ManufacturerString =%s\n", DevInformation.ManufacturerString);// 64 bytes Manufacturer string in USB descriptor
printf("FirewareVer =%d\n", DevInformation.FirewareVer);
}
}
...
//CH347SPI_SetChipSelect(fd, 0xFFFF, spiDev.iChipSelect, spiDev.iIsAutoDeativeCS,
// spiDev.iActiveDelay, spiDev.iDelayDeactive);
